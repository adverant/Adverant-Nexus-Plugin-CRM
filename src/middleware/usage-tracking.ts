/**
 * Usage Tracking Middleware for NexusCRM
 *
...
function detectOperation(req: Request): string {
  ...
  // NexusCRM specific operations
  if (path.includes('/graphql')) return 'graphql_operation';
  if (path.includes('/contacts')) return 'contact_operation';
  if (path.includes('/leads')) return 'lead_operation';
  if (path.includes('/deals')) return 'deal_operation';
  if (path.includes('/campaigns')) return 'campaign_operation';
  if (path.includes('/calls')) return 'call_operation';
  if (path.includes('/voice')) return 'voice_operation';
  if (path.includes('/webhooks')) return 'webhook_operation';
  if (path.includes('/tasks')) return 'task_operation';
  if (path.includes('/notes')) return 'note_operation';
  if (path.includes('/activities')) return 'activity_operation';
...
function isExemptPath(path: string): boolean {
  const exemptPaths = ['/health', '/healthz', '/ready', '/readiness', '/liveness', '/startup', '/metrics', '/ping', '/', '/version', '/health/ready', '/health/live'];
